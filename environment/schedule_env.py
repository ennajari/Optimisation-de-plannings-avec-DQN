...
class ScheduleEnv(gym.Env):
    ...
    def __init__(self, data_path=None, user_id=None, n_time_slots=24, max_activities=10):
        ...
        self.action_space = spaces.MultiDiscrete([len(self.activity_types), self.n_time_slots])
        ...
        self.observation_space = spaces.Dict(
            {
                ...
                "day_of_week": spaces.Box(low=0, high=1, shape=(self.days_of_week,), dtype=np.float32),
                "time_remaining": spaces.Box(low=0, high=self.n_time_slots, shape=(1,), dtype=np.float32),
            }
        )
    ...
    def load_user_data(self, data_path, user_id=None):
        ...
        try:
            ...
            print(f"Erreur lors du chargement des données: {e!s}")
    ...
    def render(self, mode="human"):
        ...
        for slot in range(self.n_time_slots):
            activities = [
                self.activity_types[i] for i in range(len(self.activity_types)) if self.current_schedule[slot, i] > 0
            ]
    ...
    def get_optimal_schedule(self):
        ...
        day_activities = day_activities.merge(self.activity_frequency, on=["ACTIVITY_NAME", "TUDIARYDAY"])
        ...
        for _, row in day_activities.iterrows():
            ...
            preferred_slot = round(preferred_time_minutes / 60) % self.n_time_slots
